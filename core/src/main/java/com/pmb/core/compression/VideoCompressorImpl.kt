...
class VideoCompressorImpl @Inject constructor(
    ...
    override suspend fun compressAndReplaceVideo(
        inputPath: String,
        compressionPercentage: Int,
        frameRate: Int
    ): Boolean = withContext(ioDispatcher) {
        ...
        try {
            val tempFile = fileManager.createTempFile("compressed", ",mp4")
            val isCompressed = compressVideo(
                inputPath,
            ...
            if (isCompressed) {
                fileManager.replaceFile(inputPath, tempFile.absolutePath)
            } else {
                false
            }
        } catch (exception: Exception) {
