...
@Composable
fun FeeDetailsScreen(navigationManager: NavigationManager) {
    val fee: Double = 20000.0
    val list = listOf(
        ("کارمزد 1" to "${fee.toCurrency()} "),
        ("کارمزد 2" to "${fee.toCurrency()} "),
        ("کارمزد 3" to "${fee.toCurrency()} ")
    )
    AppContent(
        ...
        Column(
            ...
            Row(
                ...
                Spacer(modifier = Modifier.weight(1f))
                BodyMediumText(
                    text = "60000".toDouble().toCurrency(),
                    color = AppTheme.colorScheme.onBackgroundNeutralActive
                )
                CaptionText(
                    text = "ریال",
