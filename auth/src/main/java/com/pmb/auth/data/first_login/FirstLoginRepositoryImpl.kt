...
class FirstLoginRepositoryImpl @Inject constructor() : FirstLoginRepository {
    override suspend fun firstLoginStepConfirmation(
        ...
        emit(Result.Loading)
        delay(2000)
        firstLoginStepRequest.takeIf { it.mobileNumber == accountSampleModel.mobileNumber && it.password == accountSampleModel.passWord && it.userName == accountSampleModel.userName }
            ?.let {
