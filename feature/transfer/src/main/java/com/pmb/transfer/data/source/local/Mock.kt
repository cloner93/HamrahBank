...
object Mock {
    ...
    val cardVerificationEntity = CardVerificationEntity(
        id = 1L,
        duration = 80
    )

    val fakeCardBank = CardBankEntity(
        id = 1L,
    ...
    val transferReceiptSussesEntity: TransferReceiptEntity =
        TransferReceiptEntity(
            destination = transactionClientBanksEntity.first(),
            status = ReceiptStatus.SUCCESS,
            amount = 10000000.0,
            date = 1744455441,
            paymentType = PaymentType.CARD_TO_CARD,
            message = "این رسید به معنای واریز قطعی و نهایی نیست.",
            source = TransferSourceEntity.Card(card = fakeCardBanks.first()),
