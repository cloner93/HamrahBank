package com.pmb.facilities.bill.presentation.bills_history
...
import com.pmb.facilities.bill.presentation.bills_history.viewModel.BillsHistoryViewActions
import com.pmb.facilities.bill.presentation.bills_history.viewModel.BillsHistoryViewEvents
...
fun BillsHistoryScreen(
    ...
    viewModel: BillsHistoryViewModel,
    ...
    LaunchedEffect(Unit) {
        ...
        viewModel.viewEvent.collect { event ->
            when (event) {
                is BillsHistoryViewEvents.SelectedFilterIsSuccess -> {
                    viewModel.handle(BillsHistoryViewActions.GetBillsHistory(event.filter))
                }
            }
        }
    }
    AppContent(
        ...
        topBar = {
            AppTopBar(
                title = stringResource(R.string.bills_history),
                onBack = {
            ...
            LazyRow(
                ...
                items(it) { item ->
                    ChipWithIcon(
                        ...
                        clickable = {
                            viewModel.handle(
                                BillsHistoryViewActions.SetSelectedFilter(
                                    item.filterName
                                )
                            )
                        },
